...
def initialize_parameters(
    ...
    parameters = {
        ...
        "W2": np.random.randn(100, 150) * np.sqrt(2.0 / 100),
        "b2": np.zeros((1, 150), dtype=float) * 0.01,
        "W3": np.random.randn(150, 10) * np.sqrt(2.0 / 150),
...
def plot_learning_curves(history):
    ...
    plt.text(
        last_epoch,
        last_train_loss,
        f"({last_epoch}, {last_train_loss: .4f})",
        color="black",
        ha="center",
        va="bottom",
    )
    plt.text(
        last_epoch,
        last_test_loss,
        f"({last_epoch}, {last_test_loss: .4f})",
        color="black",
        ha="center",
        va="top",
    )
    ...
    plt.title(
        f"Loss w/ size: {batch_size}, learning rate: {learning_rate}, epochs: {epochs}"
    )
    plt.xlabel("Epochs")
    ...
    plt.text(
        last_epoch,
        last_train_acc,
        f"({last_epoch}, {last_train_acc: .4f})",
        color="black",
        ha="center",
        va="bottom",
    )
    ...
    plt.title(
        f"Accuracy w/ size: {batch_size}, learning rate: {learning_rate}, epochs: {epochs}"
    )
    plt.xlabel("Epochs")
    plt.ylabel("Accuracy")
    plt.legend()
